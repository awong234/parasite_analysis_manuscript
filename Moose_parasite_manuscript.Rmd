...
Where, $y_i$ represent the parasites observed per fecal group sample, $\mu_i$ is the mean function modeled dependent upon spatial covariates $x_j(s)$ and estimated deer fecal group density $\hat\delta(s)$, and finally $\Psi_1(s)$ is a random Gaussian process with mean 0 and covariance matrix $\Sigma$. The covariance function used is the Matérn:
